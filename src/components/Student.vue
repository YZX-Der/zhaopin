  ...
  <div>
    <div class="tabbar">
      <el-menu class="el-menu-demo" mode="horizontal">
        <el-menu-item index="1" @click="studentRecruitClick">宣讲信息</el-menu-item>
        <el-menu-item index="2" @click="studentMakeClick">简历制作</el-menu-item>
        <el-menu-item index="3" @click="studentDeliverClick">简历投递</el-menu-item>
        <el-menu-item index="4" @click="studentMessageClick">
          <el-badge :value="dealMessageCount" class="item">消息通知</el-badge>
        </el-menu-item>
        <el-menu-item index="5" @click="studentChatClick">聊天信息</el-menu-item>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      dealMessageCount(){
        return this.message.length == 0 ? "" : this.message.length
      }
    },
    watch:{
      dealMessageCount(newVal,oldVal){
        if (newVal > oldVal){
          this.$notify({
            title: '您有新消息！',
            type: 'success',
            message: '您有简历通过了筛选！请前往消息页面查看！',
    ...
    methods:{
            ...
            setTimeout(() => {
              axiosRequest({
                ...
                if (successResponse.data.code == 200){
                  this.message = successResponse.data.messageList
                }
              })
              axiosRequest({
                method:'get',
                ...
                if (successResponse.data.code == 200){
                  // console.log(successResponse.data.messageList);
                  this.chat = successResponse.data.messageList
                }
              })
            },0)
        },3000)
      ...
      studentRecruitClick(){
        this.$router.replace({path:'studentRecruit',query:{username:this.username}})
      },
      studentMakeClick(){
        this.$router.replace({path:'studentMake',query:{username:this.username}})
      },
      studentDeliverClick(){
        this.$router.replace({path:'studentDeliver',query:{username:this.username}})
      },
      studentMessageClick(){
        this.$router.replace({path:'studentMessage',query:{username:this.username}})
      },
      studentChatClick(){
        this.$router.replace({path:'studentChat',query:{username:this.username}})
      },
      changePassword(){
