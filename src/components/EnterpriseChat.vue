        ...
        <el-col :span="6">
          ...
          <el-menu
            ...
            <el-menu-item :index='(index+1).toString()' v-for="(item,index) in chatFromList" :key="index" @click="itemClick(item)">
              ...
              <i class="deletePeople el-icon-close" v-show="!isDisable" @click.stop="deleteClick(index)"></i>
            </el-menu-item>
          </el-menu>
        </el-col>
...
<script>
  ...
  export default {
    ...
    // updated(){
    //   // 聊天定位到底部
    //   let ele = document.getElementById('right_body');
    ...
    methods: {
      getContact(){
        axiosRequest({
          method:'get',
          url:'/getContactByRecruiteraccount',
          params:{
            recruiteraccount:this.$route.query.username
          }
